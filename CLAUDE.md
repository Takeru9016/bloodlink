...
- **Cloud Storage security rules only cover the donor-verification path (2A-8).** `docs/FIREBASE_SETUP.md` had zero Storage rules before 2A-8 — only Firestore rules existed. 2A-8 added a rules block for `donorVerification/{userId}/*` (owner + admin only, since ID photos are more sensitive than blood group/history). The banner-carousel (`docs/SPEC.md` "Manage home carousel") and education-hub image upload paths are still unbuilt and still have no Storage rules — write them when those admin upload flows land, don't assume the donor-verification block covers them.
