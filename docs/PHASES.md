...
- [x] 4B-7 — Stale FCM token cleanup (`notifyUsers` in `functions/src/notifications/notify.ts` now clears `users/{uid}.fcmToken` on a dead-token push result — `messaging/registration-token-not-registered` or `messaging/invalid-registration-token` — via a per-token `runTransaction` that only nulls the field if it still matches the token that died, guarding against clobbering a token the client already refreshed; cleanup failures are caught and logged rather than propagated, since `notifyUsers`'s notifications batch and pushes have already gone out by that point and a thrown error would make `onRequestCreated` retry and re-send both. Deployed (`onRequestCreated`, `updateRequestStatus`) and verified live end-to-end against `bloodlink-dev-wt`: a genuine leftover dead token on a pre-existing test doc (`users/test-donor-2a7`, from the original 2A-7 investigation) triggered the real `messaging/registration-token-not-registered` path when a test `bloodRequests` doc fired the trigger, and its `fcmToken` was confirmed nulled via a direct Firestore read, with the doc's `updateTime` matching the failure log within ~150ms. Test docs cleaned up afterward — see CLAUDE.md §7)
- [x] 4B-8 — BannerItem admin-write attribution (added `updatedBy` to `BannerItemModel`, written by every write path in `banner_item_repository.dart` — `createBanner` sets it equal to `createdBy`, `updateBanner`/`setActive`/`setDisplayOrder` set it to the acting admin's uid alongside `updatedAt`; the two pre-existing live banners backfilled `updatedBy = createdBy` via `functions/scripts/backfillBannerUpdatedBy.js`, since their `updatedAt` matched their creation instant exactly — confirming `createdBy` was their only-ever writer, not a guess — confirmed via direct Firestore read before/after; `docs/DATA_MODEL.md` updated to match; see CLAUDE.md §7)

Phase 4B [ 9 / 9 tasks ] 100%
